...
int i2c_bulunan_adres;
void i2c3_scan(void)
{
	// 1'den 127'ye kadar tüm olası I2C adreslerini tarıyoruz
	for(uint8_t i = 1; i < 128; i++)
	{
		// HAL kütüphanesi adresi 1 bit sola kaydırılmış (8-bit) olarak ister
		uint16_t cihaz_adresi = (uint16_t)(i << 1);

		// Cihaza "Orada mısın?" diye sinyal gönderiyoruz
		HAL_StatusTypeDef sonuc = HAL_I2C_IsDeviceReady(&hi2c3, cihaz_adresi, 2, 10);

		// Eğer cihaz cevap verirse (HAL_OK dönerse)
		if(sonuc == HAL_OK)
		{
			i2c_bulunan_adres = i; // Bulduğumuz ham (7-bit) adresi kaydet
			break; // Ekranı bulduk, taramayı durdur!
		}
	}
}
